...
//
//  Migrado do Rodadafortuna (api/create-pix.js) — mesma lógica de
//  segurança, só troca a descrição do pagamento pro nome deste jogo.
//
...
// 1 crédito = R$ 1 nesta migração — se quiser outra proporção, ajuste aqui
// e no front (DepositScreen.tsx).
const MIN_AMOUNT = 5
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const idempotencyKey = `tigrinho-${pixRecordId}` // evita cobrança duplicada se a request for repetida
    ...
    const mpResponse = await fetch(MP_API_URL, {
      ...
      body: JSON.stringify({
        ...
        description: 'Tigrinho da Sorte - Depósito',
